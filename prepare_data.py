...
# ... (기존 코드: 데이터 로드, 결측치 제거, input_text 생성 등) ...
df = pd.read_csv(source_file)
df.dropna(subset=['english_dialogue', 'english_summary', 'english_topic'], inplace=True)
# ...
...
# 🔥 8. Sweep을 위한 훈련 데이터 서브셋(20%) 생성 추가
...
train_df.to_csv(train_file, index=False)
val_df.to_csv(val_file, index=False)
train_subset_df.to_csv(train_subset_file, index=False) # 🔥 서브셋 파일 저장
